...
- 2026-08-13 – **Drop archive/unarchive from V1** – The last V1 item with no ticket, and no requirement behind it: ADR-003 traced it to the PRD's original data model and the PRD has no such field, so nothing ever stated what archiving would do – Removed from scope rather than deferred; soft-delete does **not** absorb it; reopened only by a second user or a catalogue filtering can no longer search. V1 closes on `fe`#8, #101, #102 and #107 – [ADR-020](../ADR/ADR-020-archive-unarchive-out-of-v1.md)
- 2026-08-13 – **V2 is consultation on mobile**; Auth/Sessions/Statistics become V3 – The app is used on a phone and serves that device worst, three tickets are already parked on the layout rework, and building Practice Sessions on a layout that does not fit means building them twice – Scoped by one rule, **Consultation works everywhere / Authoring is desktop-shaped**, descriptive and never enforced by a viewport check; phone (<640px) is the only new target and tablets inherit today's layout; touch pan/pinch for scanned images, filters behind a sheet, Shot Detail content-first, app opens on the list, shell renamed off `DashboardComponent`; Diagram gestures deferred behind ADR-012's existing trigger; V1 closes on `fe`#8 and #107 with #101/#102 following V2 – [ADR-021](../ADR/ADR-021-v2-consultation-on-mobile.md)
